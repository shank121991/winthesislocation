...
\If{$distanceBtw( lst\_hr\_pts\textsubscript{i}, cluster) <=   th\_d$}
...
\For{each \textit{sp\textsubscript{i}} in \textit{sp}}
\For{each \textit{sp\textsubscript{j}} in \textit{sp}}
\If{$distanceBtw(sp\textsubscript{i}, sp\textsubscript{j}) <= th\_d$}
	...
	\If {$distanceBtw(st, newMean) <= th\_d$}
...
\begin{algorithmic}[1]
\For{each $h-hour$ from $0to24$ }
	\For{each \textit{w\textsubscript{i}} in \textit{w}}
		\For{each \textit{w\textsubscript{j}} in \textit{w}}
